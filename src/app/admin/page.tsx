  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8 space-y-10">
      ...
      <section className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="text-xs bg-red-500/10 border border-red-500/25 px-3 py-1 rounded-lg text-red-400 font-semibold tracking-wider uppercase">
          SysAdmin Portal
        </div>
      </section>
      ...
      <div className="glass-panel p-8 rounded-3xl bg-[#090d16] border border-white/10 shadow-2xl">
        ...
        {activeTab === "queue" && (
          <div className="space-y-6">
            <h3 className="font-heading text-base font-bold text-white mb-2">Pending Student Contributions</h3>
            
            {pendingUploads.length === 0 ? (
              <div className="text-center py-10 bg-white/[0.01] border border-white/5 rounded-2xl text-xs text-slate-500">
                  ...
                  <div key={item.id} className="pt-4 first:pt-0 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-slate-500 leading-relaxed">
                        Subject: <span className="text-slate-300">{item.subject}</span> • Semester: <span className="text-slate-300">{item.semester}</span> • Size: <span className="text-slate-300">{item.size}</span>
                      </p>
                      <p className="text-[10px] text-slate-600">Contributed by: {item.contributor}</p>
                    </div>

                    <div className="flex gap-2 w-full md:w-auto shrink-0">
                      <button
                        onClick={() => handleModerate(item.id, "reject")}
                        className="flex-1 md:flex-initial rounded-lg border border-white/10 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/25 px-4 py-2 text-xs font-semibold text-slate-300 transition-colors"
                      >
                        Reject
                      </button>
                      <button
        ...
        {activeTab === "reports" && (
          <div className="space-y-6">
            <h3 className="font-heading text-base font-bold text-white mb-2">Student Bug/PDF Reports</h3>
                  ...
                  <div key={report.id} className="pt-4 first:pt-0 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] font-bold uppercase px-2 py-0.5 rounded bg-amber-500/10 text-amber-500 border border-amber-500/25">
                          {report.reason}
                        </span>
                        <h4 className="text-xs font-bold text-white">{report.resourceTitle}</h4>
                      </div>
                      <p className="text-xs text-slate-400 leading-relaxed italic bg-white/[0.01] p-3 rounded-lg border border-white/5">
                        &quot;{report.details}&quot;
                      </p>
                      <p className="text-[10px] text-slate-600">Reported by: {report.reportedBy}</p>
                    </div>

                    <button
                      onClick={() => handleResolveReport(report.id)}
                      className="w-full md:w-auto shrink-0 rounded-lg border border-white/10 hover:bg-emerald-500/10 hover:text-emerald-400 hover:border-emerald-500/25 px-4 py-2 text-xs font-semibold text-slate-300 transition-colors"
                    >
                      Mark Resolved
                    </button>
                  </div>
        ...
        {activeTab === "analytics" && (
          <div className="space-y-8">
            <h3 className="font-heading text-base font-bold text-white border-b border-white/5 pb-2">
              ...
              <div className="space-y-4">
                ...
                <ul className="space-y-3 text-xs">
                  {topSubjects.map((sub, idx) => (
                    <li key={idx} className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5">
                      <div className="truncate pr-2">
                        <div className="font-bold text-slate-200 truncate">{sub.name}</div>
              ...
              <div className="space-y-4">
                ...
                <ul className="space-y-3 text-xs">
                  {topDownloads.map((dl, idx) => (
                    <li key={idx} className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5">
                      <div className="truncate pr-2">
                        <div className="font-bold text-slate-200 truncate">{dl.title}</div>
              ...
              <div className="space-y-4">
                ...
                <ul className="space-y-3 text-xs">
                  {topSearches.map((se, idx) => (
                    <li key={idx} className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5">
                      <span className="font-bold text-slate-200 truncate italic pr-2">&quot;{se.keyword}&quot;</span>
                      <span className="shrink-0 text-emerald-400 font-extrabold">{se.count} hits</span>
                    </li>
                  ))}
                </ul>
              </div>
